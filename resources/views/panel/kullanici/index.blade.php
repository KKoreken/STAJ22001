            ...
            <div class="col-12">
                <div class="card">
                    <div class="card-header">
                        <a href="{{route('panel-user-add')}}"><button  class="btn btn-success" id="btn-save-event">Yeni Ekle</button></a>
                    </div>
                    <div class="card-body">
                        <table id="datatable" class="table table-striped table-bordered dt-responsive nowrap"
                            ...
                            @foreach($users as $u)
                                <tr>
                                    <td>{{$u->name}}</td>
                                    <td>{{$u->departman->baslik}}</td>
                                    <td>
                                        @foreach($u->ekipUyeleri as $uyelik)
                                            <a href="{{route('panel-squad-details',['id'=>$uyelik->ekip->id])}}"><span class="badge badge-soft-info font-size-12">{{$uyelik->ekip->baslik}}</span> </a>
                                        @endforeach
                                    </td>
                                    <td>{{$u->created_at}}</td>
                                    <td>{{$u->tum_isler}}</td>
                                    ...
                                    <td>
                                        <div class="dropdown">
                                            ...
                                            <div class="dropdown-menu dropdown-menu-end " style="position: absolute; inset: 0px 0px auto auto; margin: 0px; transform: translate3d(-40px, 30px, 0px);" data-popper-placement="bottom-end">
                                                <a class="dropdown-item" href="{{route('panel-user-details',['id'=>$u->id])}}">İncele</a>
                                                <a class="dropdown-item" href="{{route('panel-user-edit',['id'=>$u->id])}}">Düzenle</a>

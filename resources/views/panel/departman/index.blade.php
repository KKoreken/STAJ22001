            ...
            <div class="col-12">
                <div class="card">
                    <div class="card-header">
                        <a href="{{route('panel-departmant-add')}}"><button  class="btn btn-success" id="btn-save-event">Yeni Ekle</button></a>
                    </div>
                    <div class="card-body">
                        <table id="datatable" class="table table-striped table-bordered dt-responsive nowrap"
                            ...
                            <tbody>
                            @foreach($departmants as $d)
                                <tr>
                                    <td>{{$d->baslik}}</td>
                                    <td>{{$d->created_at}}</td>
                                    <td>{{$d->ekipler_count}}</td>
                                    <td>{{$d->users_count}}</td>
                                    <td>
                                        <div class="dropdown">
                                            ...
                                            <div class="dropdown-menu dropdown-menu-end " style="position: absolute; inset: 0px 0px auto auto; margin: 0px; transform: translate3d(-40px, 30px, 0px);" data-popper-placement="bottom-end">
                                                <a class="dropdown-item" href="{{route('panel-departmant-details',['id'=>$d->id])}}">İncele</a>
                                                <a class="dropdown-item" href="{{route('panel-departmant-edit',['id'=>$d->id])}}">Düzenle</a>
